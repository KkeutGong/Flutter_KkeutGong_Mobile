...
/// Shown after the onboarding values have already been "applied" (saved to
/// SharedPreferences and confirmed against the live catalog). Recaps the
/// four picks and prompts the user to log in so the curriculum can actually
/// be generated server-side.
class OnboardingLoginPromptPage extends StatefulWidget {
  final String? certName;
  final String? examLabel;
  final String? hoursLabel;
  final String? styleLabel;

  const OnboardingLoginPromptPage({
    super.key,
    this.certName,
    this.examLabel,
    this.hoursLabel,
  ...
  @override
  State<OnboardingLoginPromptPage> createState() =>
      _OnboardingLoginPromptPageState();
}

class _OnboardingLoginPromptPageState extends State<OnboardingLoginPromptPage> {
  ...
  void initState() {
    super.initState();
    _certName = widget.certName;
    _examLabel = widget.examLabel;
    _hoursLabel = widget.hoursLabel;
    _styleLabel = widget.styleLabel;
    // If the parent didn't pre-resolve the labels (e.g. a later session
    // returns to this page), refetch them from prefs so the card never
    // shows blanks.
    if (_certName == null && _examLabel == null) {
      _loadSummary();
    }
  }
